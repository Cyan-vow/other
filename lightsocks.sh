...
Service_lightsocks(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/yeyuehuo/moran/master/service/lightsocks_centos" -O /etc/init.d/lightsocks; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/yeyuehuo/moran/master/service/lightsocks_debian" -O /etc/init.d/lightsocks; then
			echo -e "${Error} Lightsocks服务 管理脚本下载失败 !" && rm -rf "${file}" && exit 1
		fi
		chmod +x "/etc/init.d/lightsocks"
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/yeyuehuo/moran/master/lightsocks.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/lightsocks" ]]; then
		rm -rf /etc/init.d/lightsocks
		Service_lightsocks
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/yeyuehuo/moran/master/lightsocks.sh" && chmod +x lightsocks.sh
